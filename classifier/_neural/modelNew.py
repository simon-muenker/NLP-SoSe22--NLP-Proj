...
class SelectorPerceptron(nn.Module):
    ...
    def __init__(self, user_config: dict = None):
        super().__init__()
        self.config: dict = SelectorPerceptron.default_config()
    ...
    def forward(self, data) -> torch.Tensor:
        return torch.stack([
            torch.index_select(state, 0, selection)
            for state, selection in zip(
                self.perceptron(data),
                torch.floor(self.selector(data) * (self.config["in_size"] - 1)).long()
            )
        ])
